...
4. 已提供 stock detail 入口：`/api/v1/stocks/{symbol}/snapshot`、`/api/v1/stocks/{symbol}/kline`、`/api/v1/stocks/{symbol}/indicators`、`/api/v1/stocks/{symbol}/capital-flow`、`/api/v1/stocks/{symbol}/fundamentals`、`/api/v1/stocks/{symbol}/disclosures`、`/api/v1/stocks/{symbol}/corporate-actions`、`/api/v1/stocks/{symbol}/price-volume-analysis`。
...
6.1. 已提供最小策略监控入口：`GET /api/v1/strategy-watchlist`、`POST /api/v1/strategy-watchlist`、`DELETE /api/v1/strategy-watchlist/{symbol}`；当前支持把研究池内股票加入手动监控列表，并基于内置三策略输出 `BUY / WATCH / SELL`、买点、止盈位、风控线和止损位。
6.2. 已提供最小盘中监控入口：`GET /api/v1/preview/watchlist`、`POST /api/v1/preview/watchlist/run-check`；当前会基于手动监控池拉取盘中预览报价，输出实时买点/止盈/风控/止损触发状态，并把盘中提醒写入 alerts。
6.3. 已提供最小单票量价诊断入口：`GET /api/v1/stocks/{symbol}/price-volume-analysis`；它基于最新 `READY snapshot` 汇总日线、均线、量比、形态信号和资金确认，输出 `BUY / WATCH / AVOID / UNAVAILABLE` 量价状态、触发价、失效位、理由和风险。`300077.SZ` 可用 `QUANTA_SOURCE_UNIVERSE=focus_300077` 或 `QUANTA_SOURCE_SYMBOLS=300077.SZ` 纳入单票研究池后再同步。
...
10.1. 当前 workbench 的产品定位是“盘后研究驱动盘中监控”：默认用最新 `READY snapshot` 生成可解释的监控计划，再用独立盘中预览行情判断买点、止盈、风控和止损是否触发。
10.2. 这意味着在 `T+0` 白天，即使页面已经是 live runtime，正式研究口径仍来自最近一个已完成盘后发布的 `biz_date`；如果 `tushare daily/index_daily` 对当天 `trade_date` 仍返回空集，latest `READY snapshot` 会继续停留在上一个开市日。
10.3. “想看今天盘中触发状态，但又不污染 READY snapshot” 这类需求，收敛到盘中监控/预览层；需求与方案概述见 [docs/product-specs/intraday-preview-layer.md](../../docs/product-specs/intraday-preview-layer.md)。
...
26. `corporate_action_item` 会在同一 `snapshot_id` 下持久化，并通过 stock corporate-actions API 与 workbench 暴露；默认 `fixture_json` 开发链会从 `backend/app/fixtures/source_corporate_actions/` 读取本地 fixture。`tushare` 企业行为 provider 会在一次进程内按 symbol 缓存原始 `dividend` rows，并继续按每个 `biz_date` 做 as-of 过滤；长窗口回补不应再按 `symbol × biz_date` 重复拉同一批分红送配数据。
...
30. `python3 -m backend.app.domains.market_data.sync --start-biz-date YYYY-MM-DD --end-biz-date YYYY-MM-DD --print-summary` 现在已支持最小历史回补；provider 会先列出开市日，再按日期同步，并默认跳过已存在的 `biz_date`。CLI 可通过 `--artifact-mode all` 保持逐日完整发布，也可通过 `--artifact-mode latest` 让中间历史日只写 raw facts、窗口终点再发布完整 artifact。
31. `scripts/market_data_backfill_smoke.py` 会在 fake Tushare 环境里验证 `lookback window`、显式 `target start date`、`target_start_biz_date=auto`、传统逐日 artifact 回补，以及 latest artifact 回补；`scripts/tushare_live_backfill_smoke.py` 则默认使用 latest artifact 模式，在真 token 下用隔离 DuckDB 输出 `lookback_open_days/target_start_biz_date`、`history_coverage` 和 `corporate_action_check`，用来观察企业行为 reconciliation 是否随着窗口加深而推进。
32. `history_backfill` 现在也已接入 API / durable queue / worker / scheduler；scheduler 在发现最新 READY snapshot 落后于 source provider 时，会优先 enqueue `history_backfill`，而不是只同步单天。service worker 执行 `history_backfill` 时使用 latest artifact 模式，避免把多年历史回补扩成多年数量的 published snapshots。
...
39. `strategy_watchlist` 目前是面向 workbench 的最小“个股盯盘队列”：你可以手动加入股票，系统会在最新 `READY snapshot` 上自动选取最匹配的内置策略，并给出当前价、买点、止盈位、风控线、止损位与 `BUY / WATCH / SELL` 判断；当盘后新 snapshot 触发新的 `BUY` 或 `SELL` 状态时，也会写入本地 alerts JSONL，并直接出现在前端告警侧栏里。
40. `intraday preview watchlist` 现在是面向 workbench 的最小“盘中实时监控层”：它与 `READY snapshot` 分层存在，默认只盯手动监控池，并通过 experimental realtime provider 拉取盘中价、判断买点/止盈/风控/止损是否被触发；resident scheduler 每个 tick 都会顺带跑一轮盘中检查，命中后写本地 alerts JSONL，前端会自动轮询并尝试触发浏览器通知。
