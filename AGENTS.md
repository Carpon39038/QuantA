...
QuantA 当前处于绿地阶段，产品定位是用可复现的 A 股盘后研究，生成第二天盘中可执行、可监控、可风控的盯盘计划。

当前主闭环不是“盘后研究到展示为止”，而是：

1. 盘后更新数据。
2. 生成分析产物与选股结果。
3. 在真实约束下完成回测。
4. 发布可复现的盘后快照和策略监控计划。
5. 盘中基于该计划监控价格、买点、止盈、风控和止损触发。
...
5. 单写者优先。
   盘后任务负责写入正式研究产物，查询侧默认只读已发布快照；盘中监控只消费这些计划和独立预览行情，不反写正式研究结论。
6. 先补文档入口，再扩实现细节。
