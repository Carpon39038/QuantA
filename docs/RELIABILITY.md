...
4.1. `pnpm run pipeline:daemon` 是本机第一版常驻流水线入口；resident scheduler 每个 tick 会输出一行 JSON，并在 tick-level 异常时写 `scheduler_loop_failure` alert 后继续轮询。完整启动、健康检查、追深回补和停机口径见 `docs/OPERATIONS.md`。
