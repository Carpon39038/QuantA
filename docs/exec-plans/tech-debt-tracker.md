...
| --- | --- | --- | --- | --- |
| TD-001 | Repo | DuckDB-backed dev 闭环已覆盖 latest snapshot、as-of、analysis、screener、backtest、source-backed sync、最小历史回补、retry/backoff、alerts、resident scheduler、Tushare canonical provider 的离线 / live 主链、CNInfo 官方披露元数据 sidecar、canonical quality checks，以及 AKShare/BaoStock 补充校验；当前仍缺更完整的长期补数策略、更深企业行为口径和更稳的 AKShare 链路 | 当前已能稳定开发与验证更大研究池下的最小研究闭环，也能跑真实 canonical sync、财务 sidecar、官方披露 metadata、字段级 shadow validation、复权/停牌/涨跌停 canonical checks、隔离历史回补和基础调度 catch-up；离完整 v1.0 盘后生产运行仍差长期 backfill 运维策略、更深企业行为修正，以及 AKShare 上游链路稳定性 | 继续补全长期 backfill 运维策略，再把数据质量口径推进到分红送配/复权基准长期回溯，并把 AKShare 的降级信号接到更明确的通知与运行手册 |
