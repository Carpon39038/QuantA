...
| --- | --- | --- | --- | --- |
| TD-001 | Repo | DuckDB-backed dev 闭环已覆盖 latest snapshot、as-of、analysis、screener、backtest、source-backed sync、最小历史回补、retry/backoff、alerts、resident scheduler、Tushare canonical provider 的离线 / live 主链，以及 CNInfo 官方披露元数据 sidecar；当前仍缺更完整的长期补数策略和 AKShare/BaoStock 的补充校验链 | 当前已能稳定开发与验证最小研究闭环，也能跑真实 canonical sync、财务 sidecar、官方披露 metadata、隔离历史回补和基础调度 catch-up；离完整 v1.0 盘后生产运行仍差长期 backfill 运维策略与补充数据源 shadow validation | 继续补全长期 backfill 运维策略，再做 AKShare/BaoStock 的补充校验链，并把披露链扩展到正文/问询等更深层内容 |
