...
| --- | --- | --- | --- | --- |
| TD-001 | Repo | DuckDB-backed dev 闭环已覆盖 latest snapshot、as-of、analysis、screener、backtest、source-backed sync、retry/backoff、alerts 和 resident scheduler，且 AKShare 已作为既定真实数据源之一接入，但其安装、网络链路、上游公开接口稳定性与全量历史回补仍未产品化 | 当前已能稳定开发与验证最小研究闭环，但离真实盘后生产运行仍差外部源运维、外网连通性和全量历史同步 | 继续把 `akshare` 同步链推进到可安装、可回补、可校验的正式数据同步链，并补外部网络环境检查 |
